import { Bell, BellOff, Command, HelpCircle, Menu, Plus, Search } from 'lucide-react'
import { useEffect, useRef, useState, type FormEvent } from 'react'
import { Link, useNavigate } from 'react-router-dom'

import { UserAvatar } from '@/components/common/UserAvatar'
import { useNotificationReadActions, useNotifications } from '@/api/queries'
...
import { activeUploadPath, cn } from '@/lib/utils'
...
export function Header({ title, onOpenMobileNav }: HeaderProps) {
  ...
  const { data: notificationFeed, isError: notificationsError } = useNotifications()
  const { markRead } = useNotificationReadActions()
  const notifications = notificationFeed?.items ?? []
    ...
    <header className="sticky top-0 z-30 flex h-16 shrink-0 items-center gap-3 border-b border-line bg-white/85 px-4 backdrop-blur-md lg:px-8">
      ...
      <div className="flex shrink-0 items-center gap-1">
        <Button asChild variant="ghost" size="icon-sm" className="hidden sm:inline-flex">
          <Link to={activeUploadPath()} aria-label="문서 업로드">
            <Plus />
          </Link>
        </Button>
          ...
          <DropdownMenuContent className="w-[340px] p-0">
            ...
            <div className="max-h-[320px] overflow-y-auto p-1.5">
              ...
              {!notificationsError && notifications.length === 0 && (
                <div className="flex flex-col items-center gap-2 px-4 py-8 text-center text-ink-subtle">
                  <BellOff className="size-5" />
                  <span className="text-[12px]">새 알림이 없습니다.</span>
                </div>
              )}
              {notificationsError && (
                <p className="px-4 py-6 text-center text-[12px] text-danger">
                  알림을 불러오지 못했습니다.
                </p>
              )}
            </div>
