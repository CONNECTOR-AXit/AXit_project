...
import { useDeleteSubmission, useMergedDocument, useProject } from '@/api/queries'
...
export default function ProjectDetail() {
  ...
  const { data: mergedDocumentData } = useMergedDocument(projectId)
  const deleteSubmission = useDeleteSubmission(projectId ?? '')
          ...
          <>
            <Button asChild variant="gradient">
              {project.status === 'completed' ? (
                <Link to={`/projects/${project.id}/analysis/result`}>
                  <Sparkles />
                  분석 결과 보기
                </Link>
              ) : (
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent>
                ...
                <DropdownMenuItem
                  ...
                  onSelect={() => mergedDocumentData && downloadMergedDocument(mergedDocumentData.document)}
                >
                  <Download />
                  내보내기
                </DropdownMenuItem>
      ...
      <Tabs defaultValue="overview">
        ...
        <TabsContent value="overview" className="space-y-5">
          <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_360px]">
            ...
            <SectionCard
              title="참여 멤버"
              action={
                <Button variant="ghost" size="sm" className="-mt-1 -mr-2 text-primary" disabled>
                  <UserPlus />
                  멤버 초대 · 범위 외
                </Button>
              }
          ...
          >
            {documents.length > 0 ? (
              <DocumentTable
                documents={documents}
                onDelete={(submissionId) => deleteSubmission.mutateAsync(submissionId)}
              />
            ) : (
              <NoDocuments projectId={project.id} />
            )}
          </SectionCard>
        </TabsContent>
        ...
        <TabsContent value="members">
          <SectionCard
            ...
            action={
              <Button variant="primary" size="sm" className="-mt-1" disabled>
                <UserPlus />
                멤버 초대 · 범위 외
              </Button>
            }
        ...
        <TabsContent value="settings" className="space-y-5">
          <SectionCard title="프로젝트 정보" description="이름과 설명을 변경할 수 있습니다.">
            <dl className="divide-y divide-line-soft">
              ...
              <InfoRow label="공개 범위">서버 기본값 · 변경은 범위 외</InfoRow>
              <InfoRow label="자동 분석">사용 안 함 · 범위 외</InfoRow>
            </dl>
          </SectionCard>
...
function NoDocuments({ projectId }: { projectId: string }) {
  return (
    <EmptyState
      icon={FileText}
      title="업로드된 문서가 없어요"
