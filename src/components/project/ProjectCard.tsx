...
import { FileText, Folder, MoreHorizontal, Pencil, Share2, Trash2, Users } from 'lucide-react'
...
export function ProjectCard({ project, className }: ProjectCardProps) {
  ...
  return (
    <motion.div variants={staggerItem} className={className}>
      <Card className="group relative h-full transition-all duration-200 hover:-translate-y-1 hover:border-primary-200 hover:shadow-lift">
        {/*
        ...
        */}
        <Link
          ...
          className="absolute inset-0 z-0 rounded-xl focus-visible:ring-[3px] focus-visible:ring-primary/25 focus-visible:outline-none"
          aria-label={`${project.name} 프로젝트 열기`}
        />

        <div className="relative z-10 flex items-start gap-3 px-5 pt-5">
          <span
            className={cn(
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button
                variant="ghost"
                size="icon-sm"
                className="relative z-20 -mt-1 -mr-1 shrink-0 opacity-0 transition-opacity group-hover:opacity-100 focus-visible:opacity-100 data-[state=open]:opacity-100"
        ...
        <div className="relative z-10 mt-4 flex items-center gap-2 px-5">
